...
namespace Dyna
{
	EngineRenderer::EngineRenderer(EngineWindow& window, EngineDevice& device)
		:currentWindow{window},currentDevice{device}
	{
		recreateSwapChain();
	...
	void EngineRenderer::endFrame()
	{
		...
		if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR ||
			currentWindow.wasWindowResized()) {
			currentWindow.resetWindowResizedFlag();
			recreateSwapChain();
		}
	...
	void EngineRenderer::createCommandBuffers()
	{
		...
		allocInfo.commandPool = currentDevice.getCommandPool();
		allocInfo.commandBufferCount = static_cast<uint32_t>(commandBuffers.size());

		if (vkAllocateCommandBuffers(currentDevice.device(), &allocInfo, commandBuffers.data()) !=
			VK_SUCCESS) {
	...
	{
		auto extent = currentWindow.getExtent();
		while (extent.width == 0 || extent.height == 0) {
			extent = currentWindow.getExtent();
			glfwWaitEvents();
		}
		vkDeviceWaitIdle(currentDevice.device());
		SwapChain = nullptr;
		if (SwapChain == nullptr) {
			SwapChain = std::make_unique<EngineSwapChain>(currentDevice, extent);
		}
		else {
			std::shared_ptr<EngineSwapChain> oldSwapChain = std::move(SwapChain);
			SwapChain = std::make_unique<EngineSwapChain>(currentDevice, extent, oldSwapChain);
